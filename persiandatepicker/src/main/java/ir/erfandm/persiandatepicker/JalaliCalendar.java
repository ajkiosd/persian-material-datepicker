...
public class JalaliCalendar {
    ...
    public String getDayOfWeekShortString() {
        return getDayOfWeekString().substring(0, 1);
    }
    ...
    public void set(int field, int value)
    {
        if (field == Calendar.MONTH) {
            ...
            int difference = value - currentDayOfWeek;
            add(Calendar.DAY_OF_MONTH, difference);
        }
    }

    public void setTimeInMillis(Long first) {
        var gc = toGregorian();
    ...
    public void setDateFromString(String jalaliDateStr) {
        ...
        if (jalaliDateStr == null || jalaliDateStr.isEmpty()) {
            throw new IllegalArgumentException("Invalid Jalali date string");
        }

        // Remove any spaces and handle the string properly
        jalaliDateStr = jalaliDateStr.trim();
        ...
        try {
            ...
            // Update the JalaliCalendar object with the parsed values
            set(year, month, day);
